...
public class AutoCompleteMatchCase implements Dictionary, AutoComplete {
	...
	public List<String> predictCompletions (String prefix, int numCompletions) {
		...
		for (char c : prefix.toCharArray ()) {
			curr = curr.getChild (c);
			if (curr == null) return predictedWords;
		}
		...
		while (!queue.isEmpty () && predictedWords.size () < numCompletions) {
			//       remove the first Node from the queue
			TrieNode removed = queue.remove ();
			//       If it is a word, add it to the completions list
			if (removed.endsWord ()) predictedWords.add (removed.getText ());
			//       Add all of its child nodes to the back of the queue
			for (char c : removed.getValidNextCharacters ()) queue.add (removed.getChild (c));
		}
